...
def get_driver_wrapper_from_object(obj: DriverWrapper | Any) -> DriverWrapper | None:
    """
    Retrieve the driver wrapper from a given object. The object can either be a :class:`.DriverWrapper` instance
    ...
    if isinstance(obj, DriverWrapper):
        ...
        obj_nfo = f'"{obj.name}" of "{obj.__class__}"' if obj else obj
        msg = f'Cant get driver_wrapper from {obj_nfo}'
...
class DriverMixin:
    ...
    def driver_wrapper(self, driver_wrapper: DriverWrapper) -> None:
        """
        Set the driver wrapper instance, for this object.
